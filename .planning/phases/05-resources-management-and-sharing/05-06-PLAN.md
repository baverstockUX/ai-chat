...
2. Update chat API to handle search intent and image context:

In app/api/chat/route.ts, add search detection and context injection:
```typescript
...
Search triggered by keywords: "search for X", "look up X", "search: X".
  </action>
  <verify>
...
grep "searchContext" app/api/chat/route.ts
  ...
  <done>MessageInput with image upload, chat API with search intent detection and context injection</done>
</task>
